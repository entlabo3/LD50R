...
baseplot <- function(){
  ...
  plot(dose, qnorm(affected/total),
      ...
      ylab="Mortality rate (%) in Probit scale",
      xlab="Concentration"
  )  
...
## LD1,5,10,20,25,30,40,50,60,70,75,80,90,95,99
dp = dose.p(fit.probit, p = c(0.01,0.05, 0.1,0.2,0.25,0.3,0.4, 0.5,0.6,0.7,0.75,0.8, 0.9, 0.95,0.99))

LD_values <- attributes(dp)$Dose
  
## 95%信頼限界個別表示
dse.pro <- attributes(dp)$SE
p2.lw.pro <- 10 ^ (dp - dse.pro*1.96)
